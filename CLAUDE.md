...
- **Speculative decoding** — `SpeculativeDecoder` (general draft-model speculation), `MtpDecoder` + `MtpBatchTail` (self-speculative Multi-Token Prediction / NEXTN heads, e.g. Qwen3.6-27B-MTP, with folded k-token batched verify, issue #207), `PromptLookupDraft` (prompt-lookup draft), and `DSparkDecoder` + `DSparkDraftModel` (DeepSeek DSpark block-parallel safetensors draft heads, docs/dspark-plan.md / PR #413: EAGLE-3-style backbone conditioned on target hidden-state taps via `IForwardPass.EnableHiddenTaps`, rank-256 Markov re-bias, confidence-trimmed verify; greedy + CPU target only for now — `--dspark-model <safetensors> --temp 0 -g 0`, placement via `DSparkPlacementPlanner` / `--dspark-place` / `SHARPI_DSPARK_*`; fetch heads with `download-model.ps1 -Model dspark-qwen3-4b`). Toggle from the CLI (`--mtp`, `--draft-model`) or server (`SpecType`).
