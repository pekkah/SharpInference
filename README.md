...
| Carnice (Qwen3.6-35B-A3B-MTP finetune) | [mudler](https://huggingface.co/mudler/Carnice-Qwen3.6-MoE-35B-A3B-APEX-MTP-GGUF) | 17 GB | **CUDA** `-g -1 --no-thinking` (hybrid) | 17.5 | 5.3 | APEX-MTP-I-Compact mixed-precision quant: Q3_K + Q8_0 routed experts (rest Q4_K/Q5_K). Hermes-style agentic/tool-calling finetune of 35B-A3B-MTP; same qwen35moe arch + MTP head; 100% draft acceptance on this prompt. Q3_K already had a fused dequant-dot kernel; this build adds the Q8_0 dispatch arm + a first-pass AVX2 `DotQ8_0` (4× 8 f32 expand + FMA per block). Decode is well below the all-Q4_K 35B-A3B-MTP row above because the Q8_0 kernel doesn't yet match the MultiplyAddAdjacent-based throughput of `DotQ4K` — narrower AVX2 packing or VNNI-equivalent is the perf follow-up |
