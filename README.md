...
| Gemma 4 E4B-it Q8 | (same) | 8 GB | **CUDA** `-ngl -1 -c 2048` | **47.7** | **44.1** | all 42 layers fit at `-c 2048`. KV-share alias + SWA/global split per layer; PLE projections (~215 MB) upload at construction, per-token dequant + projection on GPU |
