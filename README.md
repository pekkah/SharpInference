...
| Gemma 4 E4B-it Q8 | (same) | 8 GB | **CUDA** `-g -1 -c 2048` | **1564** | **59** | all 42 layers fit at `-c 2048`. KV-share alias + SWA/global split per layer; PLE projections (~215 MB) upload at construction. **Prefill (#141):** the batched trunk runs Q8_0 matmuls as a compute-bound `cublasGemmEx` (dequant Q8_0→fp16 once per batch) and F32 PLE projections via TF32 Sgemm, instead of the memory-bound matvec GEMM-N that re-streamed each weight per token — **14×** over the #136 baseline (`SHARPI_PREFILL_GEMM=0` → 109 t/s to bisect). **Decode (#142):** dp4a/Q8_1 int8 matvec (`SHARPI_Q80_DP4A=0` to bisect) + CUDA-graph capture/replay now default-on (`SHARPI_CUDA_GRAPH=0` → 52 t/s to bisect). Both prefill GEMM and dp4a are argmax-stable vs the fp32 path, not bit-exact. Remaining gap to llama.cpp (~8000 prefill / ~78 decode) is its int8 tensor-core MMQ + flash attention |
