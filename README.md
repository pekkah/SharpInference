...
| Qwen3 8B | (same) | 5 GB | **CUDA** `-g -1` | **432** | **70.0** | Compute-bound Q4_K prefill (#156 Item C) 119.8 → 432 t/s (3.6×, same-session A/B): Q4_K trunk matmuls dequant→fp16→cuBLAS GEMM (`llm_dequant_q4k_to_f16`, weight read once per batch) instead of the memory-bound matvec GEMM-N (weight re-streamed per token); argmax-stable, `SHARPI_PREFILL_GEMM=0` to bisect. (llama.cpp b8585 pp1008 5764 t/s — the ~13× gap is its int8 Q4_K MMQ kernel, Item C2 follow-up.) Builds on batched-trunk prefill + flash attention (#156 A). Decode CUDA graphs (#158) capture/replay the per-token device region, 65 → 70 t/s (+7%), `SHARPI_CUDA_GRAPH=0` to bisect |
