...
| Qwen3 8B | (same) | 5 GB | **CUDA** `-g -1` | **2431** | **74.7** | Compute-bound Q4_K prefill (#156 Item C) 119.8 → 432 t/s (3.6×, same-session A/B). **C2 (default):** an int8 **tensor-core MMQ** matmul (`llm_mmq_q4k`, `mma.m16n8k32.s8`) reads each Q4_K weight once as int8 — nibble-expanded, `get_scale_min_k4` decode, asymmetric min-bias — with **no fp16 dequant temp to HBM**; `SHARPI_PREFILL_MMQ=0` reverts to **C1**, the dequant→fp16→cuBLAS GEMM (`llm_dequant_q4k_to_f16`); both read the weight once per batch and replace the memory-bound matvec GEMM-N (weight re-streamed per token); both argmax-stable (`SHARPI_PREFILL_GEMM=0` reverts to the bit-exact matvec). MMQ vs C1 same-session A/B: **+25% at ~100-tok prompts** (284 → 355 t/s, where C1 still pays its fp16-temp write), converging to a tie by ~1K ctx as cuBLAS amortizes that temp (430 → 432 @1008). **The 432 was then capped by mixed-quant fallback (#162):** Qwen3-8B-Q4_K_M keeps ~half of `ffn_down` + `attn_v` (and `output`) in **Q6_K**, which had no compute-bound prefill path and fell to the per-token GEMM-N matvec (weight re-streamed once per token = ~94 GB of weight reads for one N=2290 `ffn_down`, the dominant large-N cost). Adding a Q6_K (and Q5_K) `dequant→fp16→cuBLAS GEMM` prefill path (`llm_dequant_q6k_to_f16` / `_q5k_`, weight read once per batch, argmax-stable) so the whole trunk is compute-bound lifts **432 → 2431 t/s @1008 (5.6×)** — matmul time 4972 → 627 ms at N=2290. The same routing extends to both CUDA hybrid passes (`SHARPI_GDN_PREFILL_COMPUTE` / `SHARPI_HYBRID_PREFILL_COMPUTE`, default on; gated so the GDN/MTP bit-parity oracles keep the byte-exact matvec). (llama.cpp b8585 pp1008 5764 t/s — the remaining ~2.4× is its cp.async-pipelined MMQ + flash attention, which hide the weight re-read across token tiles cuBLAS amortizes via L2.) Builds on batched-trunk prefill + flash attention (#156 A). Decode CUDA graphs (#158) capture/replay the per-token device region, 65 → 70 t/s (+7%), `SHARPI_CUDA_GRAPH=0` to bisect. **Decode SoA (#156/#160, default-on; `SHARPI_Q4K_SOA=0` reverts):** a one-time **scale-pre-unpacked Q4_K repack** at upload — each 144-B super-block split into contiguous `[quants][unpacked 6-bit (scale,min) bytes][d·dmin]` regions — lets the decode matvec (`llm_matvec_q4k_soa`) and prefill MMQ (`llm_mmq_q4k_soa`) read plain bytes instead of running the per-super-block `get_scale_min_k4` unpack switch, whose dependent chain was starving memory-level parallelism. The Q4_K matvec was only ~74% of HBM peak vs the Q8_0 dp4a path's ~90% — purely this compute overhang (the 144-B block is already 16-byte aligned, unlike Q8_0's #149 funnelshift), and the repack lifts it to ~89% for **+13–15% matvec BW**. Same-session A/B: **decode 70.0 → 74.7 t/s (+7%)** and **prefill +5%** (the MMQ also benefits), **bit-identical** to the interleaved kernels (the stored scale/min integers are unchanged). #160 converted the remaining dense Q4_K readers to SoA — the N=2 MTP batched-verify matvec (`llm_matvec_q4k_n2_soa`) plus the `SHARPI_PREFILL_MMQ=0` fallbacks (GEMM-N `llm_matvec_q4k_gemm_n_soa`, dequant→fp16 `llm_dequant_q4k_to_f16_soa`) — all bit-identical, so the flag is now safe to default on for every dense Q4_K path (the Qwen3.6-27B-MTP llama.cpp byte-parity oracle still holds). Dense-only: MoE Q4_K weights stay interleaved (the repack is gated `!_isMoE` at upload) |
| Qwen3 8B | (same) | 5 GB | **CUDA** `-g -1 --no-thinking` | **2424** | **74.7** | reasoning suppressed in template; same Q4_K + Q6_K/Q5_K prefill GEMM + decode-graph + SoA path as the row above |
...
| Gemma 4 12B-it QAT Q4_0 | [google](https://huggingface.co/google/gemma-4-12B-it-qat-q4_0-gguf) | 7 GB | **CUDA** `-g -1 -c 2048` | **1778** | **54** | dense 48-layer gemma4, all bulk weights Q4_0 + tied Q6_K embedding; fits 12 GB full-offload. `attention_k_eq_v` (8 global MQA layers reuse K as V, no `attn_v`), per-layer KV heads (8 GQA / 1 MQA), pure V-norm. **Prefill (#124/#173):** Q4_0 int8 tensor-core MMQ (`mma.m16n8k32.s8`, weight read once as int8 — raw nibbles + the −8·d_w·Σq symmetric centering, no fp16 dequant temp) over a SoA Q4_0 weight repack (16-byte-aligned quants, kills the 18-byte-block qs funnelshift) wired into the batched-trunk flash-attention path — **23 → 1778 t/s** (~77×; the bare MMQ matched cuBLAS GEMM at ~1665, the SoA repack added +7%). **Decode (#124):** Q4_0 dp4a/Q8_1 int8 matvec (asymmetric −8·Σq centering) — **25 → 54 t/s** (2.1×). Argmax-stable; SoA repack bit-identical; `SHARPI_PREFILL_MMQ=0` / `SHARPI_MMQ_SOA=0` / `SHARPI_BATCHED_PREFILL=0` / `SHARPI_Q40_DP4A=0` revert. Within ~6% of llama.cpp decode (57 t/s); prefill gap (4290) is a more advanced tiled int8 kernel (cp.async pipeline), same class as Qwen3-8B/E4B |
