...
| Carnice (Qwen3.6-35B-A3B-MTP finetune) | [mudler](https://huggingface.co/mudler/Carnice-Qwen3.6-MoE-35B-A3B-APEX-MTP-GGUF) | 17 GB | **CUDA** `-g -1 --no-thinking` (hybrid) | **20.7** | **23.8** | APEX-MTP-I-Compact mixed-precision quant: Q3_K + Q8_0 routed experts (rest Q4_K/Q5_K). Hermes-style agentic/tool-calling finetune of 35B-A3B-MTP; same qwen35moe arch + MTP head; 77% draft acceptance on the `bench-carnice.ps1` 60-token prompt (the `bench-all` "capital of France" default elicits a 1-token EOS on Carnice's terser agentic tuning, so it gets its own bench cell). Numbers are apples-to-apples warm-cache; the prior 5.3 t/s decode in this row was a cold-cache artefact. Issue #101 adds `DotQ3K_Q8K_Avx2` (dual-acc-chain Q8_K-input VPMADDWD next to `DotQ6K_Q8K_Avx2`); behind `SHARPI_Q3K_Q8K=1` decode lifts to **25.8 t/s** (+8%). Issue #99 adds the Q8_0 sibling `DotQ8_0_Q8K_Avx2` reusing the same prepack; behind `SHARPI_Q8_0_Q8K=1` it's parity within noise — Q8_0 is MTP-head-only, ~3% of routed-MoE byte traffic. Both kept opt-in: `feedback_q4k_q8k_no_parity_win` showed cumulative trunk drift on Qwen3.6 even at per-kernel bit-equivalence, so default stays on the FP `DotQ3K` / `DotQ8_0` path until end-to-end argmax parity is broadly characterised |
