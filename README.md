...
| Qwen3 8B | (same) | 5 GB | **CUDA** `-g -1` | **119.8** | **70.0** | ~2.8× Vulkan prefill. Batched-trunk prefill (#156) 61.2 → 119.8 t/s (1.96×) — generalizes the Gemma 4 all-GPU batched prefill + flash attention to dense Q4_K; argmax-stable, `SHARPI_BATCHED_PREFILL=0` to bisect. Decode CUDA graphs (#158) capture/replay the per-token device region (layer loop + final norm/output), 65 → 70 t/s (+7%, same-session A/B), `SHARPI_CUDA_GRAPH=0` to bisect |
| Qwen3 8B | (same) | 5 GB | **CUDA** `-g -1 --no-thinking` | **61.8** | **70.0** | reasoning suppressed in template; same decode-graph path as the row above |
