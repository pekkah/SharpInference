...
| Gemma 4 12B-it QAT Q4_0 | [google](https://huggingface.co/google/gemma-4-12B-it-qat-q4_0-gguf) | 7 GB | **CUDA** `-g -1 -c 2048` | **1692** | **54** | dense 48-layer gemma4, all bulk weights Q4_0 + tied Q6_K embedding; fits 12 GB full-offload. `attention_k_eq_v` (8 global MQA layers reuse K as V, no `attn_v`), per-layer KV heads (8 GQA / 1 MQA), pure V-norm. **Prefill (#124):** Q4_0 dequant→fp16→cuBLAS GEMM (weight read once per batch) wired into the batched-trunk flash-attention path — **23 → 1692 t/s** (~73×). **Decode (#124):** Q4_0 dp4a/Q8_1 int8 matvec (asymmetric −8·Σq centering) — **25 → 54 t/s** (2.1×). Both argmax-stable; `SHARPI_BATCHED_PREFILL=0` / `SHARPI_Q40_DP4A=0` revert. Within ~6% of llama.cpp decode (57 t/s); prefill gap (4290) is the Q4_0 int8-MMQ kernel, same class as Qwen3-8B |
